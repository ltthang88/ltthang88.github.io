...
Một kỹ sư senior từng ngồi 6 tiếng debug một pod không chịu start. Log nhìn ổn. Manifest nhìn đúng. Mọi thứ đều "check out". Rồi có người hỏi: *"Đã xem node selector chưa?"* — Pod đang bị schedule vào nhóm node **không còn tồn tại**. Năm phút sau, nó chạy.

Kỹ sư đó không hề kém. Anh ấy biết Kubernetes, đã deploy hàng trăm ứng dụng. Nhưng anh **nhìn nhầm tầng (layer)**. Kubernetes có cái tài giấu vấn đề thật cách chỗ bạn đang nhìn tới ba lớp trừu tượng.

Những kỹ sư Kubernetes giỏi nhất tôi biết không có tool xịn hơn hay dashboard hào nhoáng hơn. Họ chỉ **bị bỏng đủ nhiều để biết bẫy nằm ở đâu**.
...
Vấn đề là Kubernetes được thiết kế bởi những người vận hành hệ phân tán khổng lồ. **Các abstraction chỉ "click" khi bạn đã từng nếm nỗi đau mà chúng giải quyết:**
...
Nếu chưa nếm những nỗi đau đó, giải pháp trông như tùy tiện.
...
- **Ngừng dùng `kubectl run` cho mọi thứ.** Viết manifest. Version chúng. Apply, delete, reapply. Production rồi cũng làm vậy.
...
Kỹ sư trung bình **biết lệnh**: deploy app, expose service, xem log. Cái đó đưa bạn đi được 70% chặng đường. Kỹ sư chuyên gia **biết các failure mode**.
...
Họ đã thấy *tất cả* những cái này, và nhận ra trong vài giây. Họ cũng biết **những gì Kubernetes sẽ không nói cho bạn**: API server vui vẻ chấp nhận một manifest hỏng hoàn toàn và schedule nó. Nó chỉ fail lúc runtime. Bạn nhận một pod `CrashLoopBackOff`, còn nguyên nhân thật là một lỗi gõ trong `ConfigMap` cách đó ba resource.
...
> **Tình huống minh họa.** Hai kỹ sư cùng gặp pod `Pending`. Người trung bình restart pod, xóa đi tạo lại, vẫn `Pending`. Người chuyên gia gõ `kubectl describe pod` đọc đúng một dòng: `0/3 nodes are available: 3 Insufficient memory`. Trong 10 giây họ biết là vấn đề tài nguyên, không phải config. Cùng một lệnh — khác nhau ở chỗ *biết phải đọc gì*.

---

## Độ phức tạp ẩn không ai cảnh báo: networking

Networking là chỗ phần lớn mọi người mắc kẹt. Trên laptop, mọi thứ chạy. Deploy lên cluster thật thì đột nhiên: service không nói chuyện được với nhau, Ingress trả 503, traffic ngoài không route vào, pod ping được IP nhưng không resolve được DNS.

Vì sao? Vì networking của Kubernetes ngầm giả định bạn hiểu: **CNI plugin** route packet thế nào, **kube-proxy** với các mode khác nhau, cấu hình **CoreDNS**, **NetworkPolicy** (mặc định chặn ra sao), và các **Service type** thực sự làm gì. Không thứ nào trong số này nằm trong hướng dẫn "getting started" — bạn học chúng khi mọi thứ vỡ trên production.

> **Tình huống minh họa.** App gọi `http://payment-service` báo lỗi không resolve được, dù `kubectl get svc` thấy service tồn tại. Đào sâu: CoreDNS pod đang `CrashLoopBackOff` vì ConfigMap sai cú pháp. Mọi DNS nội bộ cluster chết theo. Đây là kiểu lỗi "trên laptop không bao giờ gặp" — vì laptop chẳng có CoreDNS.
...
- **Pattern thiết kế controller.** Khi nào dùng operator vs Job vs CronJob, vòng reconcile xử lý xung đột ra sao.
- **Đường nâng cấp (upgrade path).** Cái gì gãy khi upgrade, API nào deprecate, test upgrade thế nào mà không giết production.
...
Đây không phải kiến thức lý thuyết. Đó là **nhận diện pattern** từ việc đã thấy nhiều thứ vỡ.

---

## Góc nhìn từ hai năm tư vấn

Sau hai năm giúp các công ty adopt Kubernetes — đủ ngành, đủ quy mô — vẫn là những lỗi giống nhau ở khắp nơi:

1. **Thất bại phổ biến nhất không phải kỹ thuật, mà là tổ chức.** Đội adopt Kubernetes vì "ai cũng làm vậy", trong khi họ không có bài toán mà Kubernetes giải. Họ chỉ thêm độ phức tạp.
2. **Over-engineering.** Đội chỉ cần một pipeline deploy đơn giản lại dựng service mesh, multi-cluster, GitOps đầy đủ. Họ tốn sáu tháng cấu hình công cụ thay vì ship tính năng.
3. **Đánh giá thấp chi phí vận hành.** Chạy Kubernetes production không miễn phí. Phải có người upgrade, patch, monitor, troubleshoot, debug xoay certificate lúc 2h sáng. Không có người đó, bạn chưa sẵn sàng.

Những công ty thành công **không dùng Kubernetes cho mọi thứ** — họ dùng cho đúng workload mà nó thực sự giúp, rồi đầu tư làm những workload đó tin cậy.
...
**Mất bao lâu để giỏi Kubernetes?** Sáu tháng dùng hằng ngày để thành thạo (competent). Hai năm kinh nghiệm production để tự tin. Năm năm để có khả năng nhận diện pattern, debug nhanh hầu hết mọi thứ.
...
**Nên học managed Kubernetes hay tự dựng?** Bắt đầu với managed (EKS, GKE, AKS) — họ lo control plane để bạn tập trung học tầng workload. Vận hành cluster học sau cũng được.
